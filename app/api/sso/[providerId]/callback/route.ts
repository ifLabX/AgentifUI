...
export async function GET(
  ...
  try {
    ...
    if (!validationResult.success) {
      ...
      return NextResponse.redirect(
        new URL(
          '/login?error=cas_validation_failed&message=认证验证失败，请重新登录',
          appUrl
        )
      );
    }
    ...
    if (!employeeNumberStr) {
      ...
      return NextResponse.redirect(
        new URL(
          '/login?error=missing_employee_number&message=无法获取学工号信息，请联系管理员',
          appUrl
        )
      );
    }
    ...
    return NextResponse.redirect(
      new URL(
        `/login?error=sso_callback_error&message=${encodeURIComponent(`SSO登录处理失败: ${errorMessage}`)}`,
        appUrl
      )
    );
  }
